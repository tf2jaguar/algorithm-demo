 ...
 * create: 2022-08-29 14:57:04
 */
public class Q1011_CapacityToShipPackagesWithinDDays {
    public static void main(String[] args) {
        Solution solution = new Q1011_CapacityToShipPackagesWithinDDays().new Solution();
    ...
    class Solution {
        public int shipWithinDays(int[] weights, int days) {
            // 这里要处理好left、right 因为最低载重 需要大于等于所有货物中最大某个货物的载重
            int left = 0, right = 1;
            for (int w : weights) {
        ...
        /**
         * 定义：运载能力为x时，需要f（x）天运完所有包裹
         * f(x）随着 x 的增加单调递减
         */
        private int f(int[] weights, int x) {
            int ans = 0;
            for (int i = 0; i < weights.length;) {
                // 货物必须按照给定的顺序装运，cap 为一次能装的最大容量
                int cap = x;
